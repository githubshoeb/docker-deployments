...
CREATE DATABASE platformdev;
CREATE DATABASE zapierdev;
...
CREATE TABLE IF NOT EXISTS policy_dependency (
    ...
    FOREIGN KEY (policy_id) REFERENCES public.policy(id) ON DELETE CASCADE
);
...
CREATE USER public_user WITH PASSWORD 'changeme';
GRANT CONNECT ON DATABASE postgres TO public_user;
GRANT USAGE ON SCHEMA public TO public_user;
GRANT SELECT,INSERT,UPDATE,DELETE ON public.policy, public.policy_dependency, public.policy_ancestor, public.attr_schema_defs TO public_user;
GRANT SELECT,INSERT ON public.policy_revision TO public_user;
GRANT USAGE,SELECT ON public.policy_revision_revision_id_seq TO public_user;